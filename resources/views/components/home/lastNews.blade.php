...
<div class=" flex flex-col sm:flex-row  gap-3  ">
    <div class="sm:w-1/2 ">
        <a href={{ route('post.show', $posts[0]->id) }}>
            <div class="w-full overflow-hidden max-w-[600px]">
                <img class="hover:opacity-90 transition-all duration-500 ease-in-out"
    ...
    <div class="sm:w-1/2 flex flex-col sm:justify-between border-b sm:border-none ">
        <a href={{ route('post.show', $posts[0]->id) }}
             class="block text-3xl font-semibold hover:text-[#ce3237] text-[#821d21] transition-colors duration-200 ease-in-out">
            {{ $posts[0]->title }}
        </a>
...
<div class="flex flex-col gap-3 sm:flex-row my-8">
    <div class="sm:w-1/2 lg:w-1/3 border-b sm:border-none pb-3">
        ...
        <a href={{ route('post.show', $posts[1]->id) }}
            class="text-2xl font-semibold hover:text-[#ce3237] text-[#821d21] transition-colors duration-200 ease-in-out">
            {{ $posts[1]->title }}
        </a>
